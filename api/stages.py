    ...
    *,
    ...
    for scene in story.get("scenes") or []:
        for shot in scene.get("shots") or []:
            ...
            # S3 video route (dry by default — push metadata for gpu-host)
            video_payload = {
                "shot_id": shot.get("id"),
                "host_key": "gpu-host",
                "base_url": gpu-host_url,
                "prompt": prompt,
                "duration_s": duration_seconds(fields.get("duration") or 5),
                "status": "dry_run" if dry_run else "planned",
                "ok": True,
                "note": "Video graphs require gpu-host Comfy + Wan/I2V workflow pin; v1 records plan + probe",
            }
            if not dry_run:
                probe = comfy.probe(gpu-host_url)
