...
| **0.6.0** | 2026-08-15 | `2026.08-mok-tua-music-video-lipsync-pipeline` | MiniMax H3 and LTX-2.3 video confirmed working end to end on a 16 GB GPU; Director's Console job submission verified end to end (real file on disk, not just a success response); `curate` tool for picking best takes across runs and assembling them; two real lip-sync proof clips live on the site — LTX-2 22B (distilled 1.1), audio-conditioned on the song's real WAV track, not the dedicated lipsync tools (those are separately wired in but still unproven). |
| **0.7.0** | 2026-09-02 | `2026.09-mok-tua-tts-backends-cloud-space-earmarks` | PocketTTS + DramaBox added as local voice-clone TTS backends, each proven on a real generated wav (DramaBox confirmed GPU-near-exclusive by an actual OOM, then fixed by excluding it from the concurrent launch chain); new HF Spaces category-page puller (`sync_hf_spaces_by_category.py`, 300+ Spaces across video-generation/voice-cloning/music-generation); 11 Spaces earmarked as optional cloud-call tools, none wired in as a dependency — live-testing found 2 of 4 checked were down or incompatible, documented rather than assumed working. |
| **ops** | 2026-09-12 | `2026.09-mok-tua-h3-vocallock-lipsync-smoke` | MiniMax H3 VocalLock_V3 lipsync PASS on isolated ComfyUI (verse batch 5/5 scenes, 32s window, 16 GB GPU). Distinct from the 0.6.0 LTX-2 audio-conditioned proof and from still-unproven DreamTalk/LivePortrait/FaceFusion. Public citations for H3 / LightX2V / Vocal Lock / comfy-mcp robustness in `docs/reports/SMOKE_TESTED_CAPABILITIES_2026-09-12.md`. |
