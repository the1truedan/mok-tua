...
Mac-created dirs under `/Volumes/ai-data/work/framepack` often show as **uid 501** on gpu-host with mode `2755` (group **no write**). Linux `redacted` is **uid 1000** / **gid 1000**, so you get:
...
# prefer 2775 on dirs so setgid keeps group=redacted
...
1. Launch with wrapper on gpu-host  
