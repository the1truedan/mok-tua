# Incident — I2V provenance ambiguity (Grok Imagine vs MRGPU local)
...
An operator asked for **local MRGPU** image/video proof (GPU util, timed model/prompt evidence, identity path).  
The first “demo video” packet was labeled as if it exercised the **local stack**, but the **generative** I2V segments were produced by **Grok Imagine** (`image_to_video` in the agent session) and **ffmpeg** only. **MRGPU GPU util stayed near zero** during that path — correctly, because nothing was queued on ComfyUI / Wan / AnimateDiff.
...
|-------|----------------|-----------------|
| A | Comic stills `manager_updates_…` | **ComfyUI DreamShaper_8** on MRGPU (local stills) |
...
| D | Corrective smoke `manager_mrgpu_local_render_20260805` | **Comfy stills + AnimateDiff** on **RTX 4060 Ti**, ~48 s e2e, GPU peak **100%** |
...
4. Handoffs listed deliverables by filename/duration **before** renderer/GPU evidence, so readers assumed MRGPU.
...
| Public flip narrative | Medium — hybrid clip is valid **QQQ1 cloud** evidence, not MRGPU proof |
...
|-------|------|------------|
| **Hybrid (cloud I2V)** | `work/social-staging/2026-08/manager_vibe_demo_20260805T1620/manager_vibe_demo_hybrid_12s.mp4` | Grok Imagine + ffmpeg — **no** MRGPU generative |
| Local kenburns only | same dir `manager_vibe_demo_local_kenburns.mp4` | CPU stitch of stills |
| **MRGPU generative** | `work/social-staging/2026-08/manager_mrgpu_local_render_20260805/` | Comfy + AnimateDiff · receipt `receipts/mrgpu_local_render.json` |
| Report (local) | `docs/reports/MRGPU_LOCAL_RENDER_SMOKE_2026-08-05.md` | Corrective |
...
1. Dual-path docs: hybrid vs MRGPU labeled; hybrid banner says **not MRGPU**.  
2. MRGPU smoke report with wall-clock, VRAM, util, models, prompts, seeds.  
...
```text
renderer:   mrgpu_comfy_animatediff | mrgpu_comfy_wan | mrgpu_framepack | grok_imagine_i2v | ffmpeg_kenburns | other
qqq:        QQQ0 | QQQ1
...
| Say | Don’t say |
|-----|-----------|
| “MRGPU AnimateDiff smoke (GPU 100%)” | “local demo video” without renderer |
| “Grok Imagine I2V (QQQ1 cloud)” | “lab I2V” for cloud path |
| “ffmpeg kenburns of stills (CPU)” | “local video gen” for zoompan |

### Agent protocol

1. If the user says **MRGPU / local / GPU** → queue **Comfy / FramePack / Wan** only; do **not** call session `image_to_video` unless they also say cloud/Grok.  
...
- Public packet: hybrid may ship as optional cloud demo; **MRGPU path is the local proof**
