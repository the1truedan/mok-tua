...
The canonical MRGPU paths are:

```text
/mnt/ai-data/pinokio
/mnt/ai-data/pinokio/api
/mnt/ai-data/uv-cache/mrgpu
/mnt/ai-data/uv-cache/mrgpu/pip
/mnt/ai-data/hf-cache
/mnt/ai-data/models
```

The MRGPU SSH audit on 2026-08-04 found `dtm` UID/GID 1000 and effective
...
cache. Keep MRGPU and M4RV cache roots separate, use `UV_LINK_MODE=copy` on
...
|---|---|---|---|
| Still/image | Stability Matrix Comfy on M4RV; Qwen edit graph | primary, path present | `/system_stats`, `/object_info`, one 512px local render + output hash |
| Video/I2V | MRGPU Comfy Wan pin or Wan2GP | staged; submit contract incomplete | low-VRAM 8–16 frame clip, poll/collect receipt, no cloud |
...
1. Confirm the MRGPU Pinokio and host-split UV paths with the bounded SSH
...
3. **T1 video:** use that still for a short MRGPU I2V clip; require submit,
