#!/usr/bin/env python3
"""Regenerate CEO storyboard + face polish + AnimateDiff strip on MRGPU Comfy.
...
def build_animatediff_graph(
    *,
    ...
    """Minimal ADE Gen1 + VHS h264 — same family as prior MRGPU smoke."""
    return {
...
def collage_storyboard(panel_paths: list[Path], out_path: Path, labels: list[str]) -> None:
    ...
    cite = (
        "mok-tua · MRGPU Comfy · DreamShaper_8 · img2img from 00-ceo-source-still · QQQ0 · prompt-locked identity"
    )
...
def face_polish_sheet(before: Path, after: Path, out_path: Path) -> None:
    ...
    cite = "mok-tua · MRGPU Comfy img2img refine · DreamShaper_8 · source 00-ceo-source-still · QQQ0"
...
def extract_frame_strip(mp4: Path, out_path: Path, n: int = 4) -> bool:
    ...
    draw.text(
        (8, h + 16),
        "MRGPU AnimateDiff · DreamShaper_8 + mm_sd_v15_v2 · CEO prompt · QQQ0 · not Grok cloud",
        fill=(180, 200, 220),
        font=font,
    )
...
def main() -> int:
    ...
    rec_sb = build_receipt(
        sheet,
        renderer="mrgpu_comfy_img2img_storyboard",
        provider="comfy",
    ...
    rec_fp = build_receipt(
        polish_out,
        renderer="mrgpu_comfy_img2img_face_polish",
        provider="comfy",
        cloud_or_local="local",
        model=CKPT,
        host_role="gpu-host",
            ...
            try:
                ...
                if strip.is_file():
                    write_receipt(
                        build_receipt(
                            strip,
                            renderer="mrgpu_comfy_animatediff_frame_strip",
                            provider="comfy",
                            cloud_or_local="local",
                            model=f"{CKPT}+mm_sd_v15_v2.fp16",
                            host_role="gpu-host",
