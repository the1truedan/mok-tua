...
COMFY_URL = os.environ.get("COMFY_URL", os.environ.get("COMFY_gpu-host_URL", "http://gpu-host:8188")).rstrip("/")
...
def main() -> int:
    ...
    rec_sb = build_receipt(
        sheet,
        renderer="gpu-host_comfy_faceid_storyboard",
        provider="comfy",
    ...
    rec_fp = build_receipt(
        polish_out,
        renderer="gpu-host_comfy_faceid_face_polish",
        provider="comfy",
        cloud_or_local="local",
        model=f"{CKPT}+{FACEID_BIN}",
        host_role="gpu-host",
            ...
            if meta and str(meta.get("filename", "")).endswith(".mp4"):
                try:
                    ...
                    rec_ad = build_receipt(
                        ...
                        renderer="gpu-host_comfy_faceid_animatediff",
                        provider="comfy",
                        cloud_or_local="local",
                    ...
                    if strip.is_file():
                        write_receipt(
                            build_receipt(
                                strip,
                                renderer="gpu-host_comfy_faceid_animatediff_frame_strip",
                                provider="comfy",
                                cloud_or_local="local",
                                model=f"{CKPT}+mm_sd_v15_v2.fp16+{FACEID_BIN}",
                                host_role="gpu-host",
    ...
    if os.environ.get("SKIP_HERO", "").strip() in ("1", "true", "yes"):
        ...
        write_receipt(
            build_receipt(
                hero,
                renderer="gpu-host_ceo_hero_compose",
